...
COPY scripts/wechat-window-adapter.sh /usr/local/bin/wechat-window-adapter
RUN chmod +x /usr/local/bin/resolve-wechat-download /usr/local/bin/wechat-window-adapter
...
RUN echo '#!/bin/sh' > /startapp.sh && \
    echo '/usr/local/bin/wechat-window-adapter &' >> /startapp.sh && \
